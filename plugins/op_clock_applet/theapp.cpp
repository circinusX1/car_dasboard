...
void TheApp::init(MySett& s)
{
    ...
    setOrganizationName("zirexix");
    setApplicationName("dashcarowa");
    ...
    QTimer::singleShot(600, this, SLOT(slot_namage_apps()));
    _wm_wins_state = PS_NORMAL;
    MySett::config().finalize();
    _runicon.load_image(CFG(_theme),"run-icon.png");
    QTimer::singleShot(800, this, SLOT(slot_refresh()));
    _ipc.start();
}
...
bool TheApp::is_process( const XwnSet& xset)
{
    ...
    QString syscmd = "ps ax | grep " + xset._pname +" | grep -v grep";
    QString cmdrv = _popen(syscmd, xset._pname);
    if(!cmdrv.isEmpty())
    {
...
{
    QTimer::singleShot(2000, this, SLOT(slot_normal_cursor()));
...
{
     QTimer::singleShot(3000, this, SLOT(slot_normal_cursor()));
    setOverrideCursor(Qt::WaitCursor);

    if(_is_running(xset))
    {
        QString cmd = "showapp.sh " + xset._pname + " 1";
        qDebug() << "RUN: " << cmd;
        system (cmd.toUtf8());
        return;
    }
    QString cmd = xset._cmd;
    if(!cmd.endsWith("&"))
        cmd += " &";
    qDebug() << "RUN: " << cmd;
    system(cmd.toUtf8());
...
bool TheApp::x_destroy(XEvent *event)
{
    ...
    if (pxc)
    {
        ...
        _papp_man->set_active_xwin(NULL, false);
        pxc->close();
        QTimer::singleShot(500, this, SLOT(slot_focus_topmost()));
        return true;
    }
...
//------------------------------------------------------------------------------
void TheApp::restack(XClient *pxc)
{
    size_t     pnls = 0;
    Window  wins[16];
    for(pnls=0; pnls<_panels.size(); ++pnls)
    {
